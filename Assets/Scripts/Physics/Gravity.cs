...
public class GravityZone : MonoBehaviour
{
    ...
    private void InitializeGravityRange()
    {
        ...
        {
            Debug.LogError($"{nameof(GravityZone)} requiere dos CircleCollider2D.");
            enabled = false;
            return;
        }

        maxDistance = colliders[1].radius * transform.localScale.x;
    }
    ...
    private void OnTriggerStay2D(Collider2D other)
    {
        ...
        if (controller != null)
        {
            controller.AddGravityForce(force);
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        ...
        if (controller != null)
        {
            controller.gravityForce = Vector2.zero;
        }
    }
